// Auto-generated feature route: anomaly-stream
// Domain: delivery anomaly detector — Capability: streaming detection of failed attempts and driver behavior anomalies
// Mounted under: /api/anomaly-stream
...
// POST /api/anomaly-stream/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a delivery anomaly detector. Your task is streaming detection of failed attempts and driver behavior anomalies. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/anomaly-stream/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/anomaly-stream/:id — fetch one
...
// DELETE /api/anomaly-stream/:id — discard a run
