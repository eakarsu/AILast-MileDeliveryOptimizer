...
// Route: /api/gap-proof-of-delivery
// Feature: Proof Of Delivery
// Description: Proof-of-delivery capture (signature/photo workflow)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Proof Of Delivery. Proof-of-delivery capture (signature/photo workflow)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Proof Of Delivery\nDescription: Proof-of-delivery capture (signature/photo workflow)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['proof-of-delivery', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'proof-of-delivery', title: 'Proof Of Delivery', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'proof-of-delivery', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['proof-of-delivery']);
      return res.json({ slug: 'proof-of-delivery', items: r.rows || [] });
    }
    return res.json({ slug: 'proof-of-delivery', items: [] });
  } catch (e) {
    return res.json({ slug: 'proof-of-delivery', items: [], error: (e && e.message) || 'Server error' });
  }
});
