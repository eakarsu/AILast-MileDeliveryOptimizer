// Auto-generated feature route: delivery-planner-agent
// Domain: last-mile delivery planner — Capability: real-time re-routing on traffic, weather, and failures
// Mounted under: /api/delivery-planner-agent
...
// POST /api/delivery-planner-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a last-mile delivery planner. Your task is real-time re-routing on traffic, weather, and failures. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/delivery-planner-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/delivery-planner-agent/:id — fetch one
...
// DELETE /api/delivery-planner-agent/:id — discard a run
