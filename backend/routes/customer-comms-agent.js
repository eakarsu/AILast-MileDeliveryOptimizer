// Auto-generated feature route: customer-comms-agent
// Domain: autonomous customer comms agent — Capability: drafting proactive delay notifications to customers
// Mounted under: /api/customer-comms-agent
...
// POST /api/customer-comms-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a autonomous customer comms agent. Your task is drafting proactive delay notifications to customers. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/customer-comms-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/customer-comms-agent/:id — fetch one
...
// DELETE /api/customer-comms-agent/:id — discard a run
