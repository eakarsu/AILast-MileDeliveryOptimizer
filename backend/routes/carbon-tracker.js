// Auto-generated feature route: carbon-tracker
// Domain: logistics carbon analyst — Capability: tracking carbon impact and recommending offset integrations
// Mounted under: /api/carbon-tracker
...
// POST /api/carbon-tracker/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a logistics carbon analyst. Your task is tracking carbon impact and recommending offset integrations. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/carbon-tracker/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/carbon-tracker/:id — fetch one
...
// DELETE /api/carbon-tracker/:id — discard a run
