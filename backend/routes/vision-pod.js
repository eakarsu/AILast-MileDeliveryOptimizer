// Auto-generated feature route: vision-pod
// Domain: proof-of-delivery vision validator — Capability: validating package, recipient, and location from photo
// Mounted under: /api/vision-pod
...
// POST /api/vision-pod/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a proof-of-delivery vision validator. Your task is validating package, recipient, and location from photo. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/vision-pod/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vision-pod/:id — fetch one
...
// DELETE /api/vision-pod/:id — discard a run
