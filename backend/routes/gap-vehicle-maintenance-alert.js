...
// Route: /api/gap-vehicle-maintenance-alert
// Feature: Vehicle Maintenance Alert
// Description: (predictive fleet maintenance)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Vehicle Maintenance Alert. (predictive fleet maintenance)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Vehicle Maintenance Alert\nDescription: (predictive fleet maintenance)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['vehicle-maintenance-alert', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'vehicle-maintenance-alert', title: 'Vehicle Maintenance Alert', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'vehicle-maintenance-alert', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['vehicle-maintenance-alert']);
      return res.json({ slug: 'vehicle-maintenance-alert', items: r.rows || [] });
    }
    return res.json({ slug: 'vehicle-maintenance-alert', items: [] });
  } catch (e) {
    return res.json({ slug: 'vehicle-maintenance-alert', items: [], error: (e && e.message) || 'Server error' });
  }
});
