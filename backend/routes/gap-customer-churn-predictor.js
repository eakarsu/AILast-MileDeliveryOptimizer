...
// Route: /api/gap-customer-churn-predictor
// Feature: Customer Churn Predictor
// Description: (high-value customer churn)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Customer Churn Predictor. (high-value customer churn)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Customer Churn Predictor\nDescription: (high-value customer churn)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['customer-churn-predictor', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'customer-churn-predictor', title: 'Customer Churn Predictor', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'customer-churn-predictor', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['customer-churn-predictor']);
      return res.json({ slug: 'customer-churn-predictor', items: r.rows || [] });
    }
    return res.json({ slug: 'customer-churn-predictor', items: [] });
  } catch (e) {
    return res.json({ slug: 'customer-churn-predictor', items: [], error: (e && e.message) || 'Server error' });
  }
});
