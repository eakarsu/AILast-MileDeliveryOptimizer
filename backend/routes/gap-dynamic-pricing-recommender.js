...
// Route: /api/gap-dynamic-pricing-recommender
// Feature: Dynamic Pricing Recommender
// Description: (shipping cost optimization)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Dynamic Pricing Recommender. (shipping cost optimization)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Dynamic Pricing Recommender\nDescription: (shipping cost optimization)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['dynamic-pricing-recommender', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'dynamic-pricing-recommender', title: 'Dynamic Pricing Recommender', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'dynamic-pricing-recommender', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['dynamic-pricing-recommender']);
      return res.json({ slug: 'dynamic-pricing-recommender', items: r.rows || [] });
    }
    return res.json({ slug: 'dynamic-pricing-recommender', items: [] });
  } catch (e) {
    return res.json({ slug: 'dynamic-pricing-recommender', items: [], error: (e && e.message) || 'Server error' });
  }
});
